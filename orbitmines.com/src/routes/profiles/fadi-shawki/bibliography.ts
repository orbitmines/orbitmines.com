/**
 * Everything read, watched, worked at and attended — the bibliography the
 * articles cite from.
 *
 * It was in `fadi_shawki.ts` next to the profile, and that turned out to be
 * expensive in a way that had nothing to do with either of them. The profile is
 * what `references.tsx` names in order to put an author on a paper, so every
 * paper on the site imported this module; the profile's `content` pointed at a
 * dozen entries in here, which kept the whole four thousand line literal alive;
 * and so every article shipped the complete bibliography in order to print one
 * name under its title.
 *
 * Split, the profile is a few lines and this is imported by the three places
 * that actually cite from it. Nothing in here changed in the move.
 */
