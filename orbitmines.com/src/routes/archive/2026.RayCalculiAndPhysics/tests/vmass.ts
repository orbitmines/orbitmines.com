...
console.log("m_eff ∝ v^(−q), SOLVED SELF-CONSISTENTLY ON THE RELAXED GALAXY");
console.log("=".repeat(78));
console.log("  q = 0 is no feedback.  q = 1 is the model's own massFor(v) = c/v.");
console.log("  analytic expectation for the flat channel: BTFR slope = 2 + q\n");
console.log("      q      shape    BTFR    2+q     v(8)   v(20)  v(30)");
for (const q of [0, 0.5, 1.0, 1.5, 1.85, 2.0, 2.5]) {
  const r = scoreV(q);
  const good = r.shape < 6 && Math.abs(r.btfr - 3.85) < 0.25;
  console.log(`  ${q.toFixed(2).padStart(6)}   ${r.shape.toFixed(1).padStart(5)}%  ` +
    `${r.btfr.toFixed(2).padStart(6)}  ${(2 + q).toFixed(2).padStart(5)}   ` +
    `${kms(r.g[idx(8)], ri[idx(8)]).toFixed(0).padStart(5)}  ` +
    `${kms(r.g[idx(20)], ri[idx(20)]).toFixed(0).padStart(5)}  ` +
    `${kms(r.g[idx(30)], ri[idx(30)]).toFixed(0).padStart(5)}` +
    `${good ? "   <<< PASSES BOTH" : ""}`);
}
