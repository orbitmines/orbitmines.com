...
console.log("1. THE CURVE AT q = 1, AGAINST GAIA, RADIUS BY RADIUS");
console.log("=".repeat(78));
{
  const r = scoreV(1.0);
  console.log("   r kpc   Newton   model(q=1)   Gaia    ratio");
  for (const rk of [4,6,8,10,12,15,18,20,22,25,28,30]) {
    const gN = solveV(MW, 0, 0)[idx(rk)];
    console.log(`  ${String(rk).padStart(6)}  ${kms(gN,ri[idx(rk)]).toFixed(1).padStart(7)}  `+
      `${kms(r.g[idx(rk)],ri[idx(rk)]).toFixed(1).padStart(10)}  ${MEAS(rk).toFixed(1).padStart(6)}  `+
      `${(kms(r.g[idx(rk)],ri[idx(rk)])/MEAS(rk)).toFixed(3)}`);
  }
}

console.log();
console.log("=".repeat(78));
console.log("2. HOW MUCH OF THE BTFR IS MY GALAXY-FAMILY ASSUMPTIONS?");
console.log("=".repeat(78));
console.log("  The family assumed R ∝ M^0.35 and a fixed gas fraction. Real dwarfs");
console.log("  are gas-RICH, which steepens the measured relation. Both are my");
console.log("  choices, not the model's, so their effect is a systematic:\n");
const scaled2 = (f:number, s:number, gasTilt:number): Galaxy => {
  const gf = Math.pow(f, -gasTilt);                  // gas fraction rises for dwarfs
  return { Md: MW.Md*f, Rd: MW.Rd*Math.pow(f,s),
           Mg: MW.Mg*f*gf, Rg: MW.Rg*Math.pow(f,s),
           Mb: MW.Mb*f, ab: MW.ab*Math.pow(f,s) };
};
const btfrWith = (q:number, lambda:number, s:number, gasTilt:number) => {
  const pts:[number,number][] = [];
  for (const f of [1e-2,1e-1,1,1e1,1e2]) {
    const gal = scaled2(f,s,gasTilt), gg = solveV(gal,q,lambda);
    const rf = Math.min(4*gal.Rd, ri[NOUT-1]*0.95);
    const k = Math.max(0,Math.min(NOUT-1,Math.round(rf/(0.5*KPC))-1));
    pts.push([Math.log10((gal.Md+gal.Mg+gal.Mb)/MSUN), Math.log10(Math.max(1e-6,kms(gg[k],ri[k])))]);
  }
  const n=pts.length, sx=pts.reduce((a,p)=>a+p[1],0), sy=pts.reduce((a,p)=>a+p[0],0);
  const sxx=pts.reduce((a,p)=>a+p[1]*p[1],0), sxy=pts.reduce((a,p)=>a+p[0]*p[1],0);
  return (n*sxy-sx*sy)/(n*sxx-sx*sx);
};
const lam1 = scoreV(1.0).lambda;
console.log("   size exp s   gas tilt   BTFR slope at q=1");
for (const s of [0.2,0.35,0.5]) {
  for (const gt of [0,0.15,0.3]) {
    console.log(`   ${s.toFixed(2).padStart(9)}   ${gt.toFixed(2).padStart(8)}   ${btfrWith(1.0,lam1,s,gt).toFixed(2).padStart(10)}`);
  }
}
console.log();
console.log("  measured: 3.85 +/- 0.09");
