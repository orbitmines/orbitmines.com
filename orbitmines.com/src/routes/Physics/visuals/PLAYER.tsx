      ...
      <CanvasView animate deps={[epoch, s.note]} paint={() => {
        ...
        return {
          ...
          frame: (sur: Surface, dt: number) => {
            ...
            let R = 1;
            w.backend.forEachLocal(k => {
              const p = w.backend.position(k).map(x => x - C) as Vec;
...
/** two blocks side by side, each holding one polarity, moving towards each other */
export const blocks = (left: 1 | -1, right: 1 | -1): Seed => (w) => {
  const C = ((w.opts.N ?? 1) - 1) / 2, half = Math.max(2, Math.floor(C / 2));
  w.add({ at: [C - half, C], radius: Math.max(1, Math.floor(half / 2)),
    emits: left, duty: 1, propulsion: "forward", toward: [1, 0], absorbs: true });
  w.add({ at: [C + half, C], radius: Math.max(1, Math.floor(half / 2)),
    emits: right, duty: 1, propulsion: "forward", toward: [-1, 0], absorbs: true });
};
...
export const Arrangements = ({ height = 240 }: { height?: number } = {}) => {
  ...
  const slides: Slide[] = [
    { key: "opposite", label: "two blocks, opposite polarity — they meet and annihilate",
      render: () => <Player height={height} world={plane} seed={blocks(1, -1)} warm={5} /> },
    { key: "alike", label: "two blocks, alike — they turn away from each other",
      render: () => <Player height={height} world={plane} seed={blocks(1, 1)} warm={15} /> },
