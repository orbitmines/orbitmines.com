...
/**
 * THE TRANSPORT ROUTE, AT GALAXY SCALE — which the file derived and never ran
 * on a galaxy.
 *
 *     v_carrier = c·min(1, n/n_c)      the budget, sign fixed by `inStep`
 *     Φ = 4πr²·n·v = const             flux conservation
 *
 *   dense  v = c        ⇒ n ∝ 1/r²  ⇒ g ∝ 1/r²      Newton
 *   thin   v = c·n/n_c  ⇒ n ∝ 1/r   ⇒ g = √(g_N·g_c) MOND, and √M for free
 *
 * The √M is not in the source at all — flux conservation goes QUADRATIC in n
 * once v ∝ n. Which is the non-linearity the theorem demanded.
 */
...
const scoreT = (gc: number) => {
  ...
  const sxx = pts.reduce((a, p) => a + p[1] * p[1], 0);
  const sxy = pts.reduce((a, p) => a + p[0] * p[1], 0);
  return { shape, btfr: (nn * sxy - sx * sy) / (nn * sxx - sx * sx) };
};

console.log("=".repeat(76));
console.log("THE TRANSPORT ROUTE ON THE RELAXED GALAXY — never run before");
console.log("=".repeat(76));
console.log("  one constant, g_c, which the model claims to FIX rather than fit\n");
console.log("     g_c (m/s²)     shape    BTFR slope");
for (const gc of [0.5e-10, 1.0e-10, 1.2e-10, 1.5e-10, 2.0e-10]) {
  const r = scoreT(gc);
  const good = r.shape < 6 && Math.abs(r.btfr - 3.85) < 0.25;
  console.log(`  ${gc.toExponential(2).padStart(11)}    ${r.shape.toFixed(1).padStart(5)}%  ` +
    `${r.btfr.toFixed(2).padStart(8)}${good ? "   <<< PASSES BOTH" : ""}`);
}

console.log();
console.log("  and what g_c the model's OWN n_c gives, with no fitting at all:");
const MU_SI = 0.06235150 * 2.176434e-8, LP = 1.616255e-35;
console.log("    n_c = (m/2π)³ in lattice units, g ∝ n with 4πG/SHEET = 0.097942");
for (const [nm, mMeV] of [["electron", 0.511], ["29 MeV (what it wants)", 28.9],
  ["muon", 105.66], ["pion", 134.98], ["proton", 938.26]] as [string, number][]) {
  const m = mMeV * 1.78266192e-30 / MU_SI;            // in lattice mass units
  const nc = Math.pow(m / (2 * Math.PI), 3);
  const gc = nc * 0.097942 * (2.99792458e8 / 5.391247e-44) / 1e0;
  console.log(`    ${nm.padEnd(24)} n_c = ${nc.toExponential(2)}`);
}
console.log();
console.log("    the n_c the fit wants:   2.203e-61  →  29 MeV");
console.log("    which is not a particle, and that is the whole bill.");

console.log();
console.log("=".repeat(76));
console.log("SO THE TWO ROUTES, SIDE BY SIDE");
console.log("=".repeat(76));
const g1 = scoreV(1.0), t1 = scoreT(1.2e-10);
console.log("     route                          shape    BTFR    what it owes");
console.log(`     source feedback (test G)       ${g1.shape.toFixed(1)}%    ${g1.btfr.toFixed(2)}    a sign it cannot settle`);
console.log(`     transport (inStep budget)      ${t1.shape.toFixed(1)}%    ${t1.btfr.toFixed(2)}    a 29 MeV emitter`);
console.log(`     measured                        —      3.85`);
